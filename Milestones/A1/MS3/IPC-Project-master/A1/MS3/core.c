...
int inputIntRange(int lowest, int highest) {
    ...
    do {
        ...
        if (m != '\n') {
            ...
            printf("ERROR! Value must be between %d and %d inclusive: ",lowest,highest);
        }
...
void inputCStringNumbers(char* stringOfCharacters, int minumumValue, int maximumValue) {
    ...
    while (f) {
        ...
        while (charactervalues != '\n' && g <= maximumValue) {

            charactervalues = getchar();


            stringOfCharacters[g] = charactervalues;
        ...
        if (charactervalues == '\n' && g <= (maximumValue + 1)) {
            ...
            stringOfCharacters[g] = '\0';
        }


        else {

            stringOfCharacters[maximumValue] = '\0';

            clearInputBuffer();
        }


        if (minumumValue == maximumValue && g != minumumValue) {
